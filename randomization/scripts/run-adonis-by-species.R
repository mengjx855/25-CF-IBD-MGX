...
if (length(args) < 2) {
  message("Usage: Rscript mjx-adonis-by-species.R [fam.tpm] [out_prefix]")
...
result <- map_dfr(
  proj_name, ~ {
    data_x <- data.table::fread(
      paste0(file_base, .x, '.kk2.s.gz')
    ) |> 
      column_to_rownames('name')
    
    data_y <- data.frame(na.omit(tpm[, colnames(data_x)]))
    data_y <- data_y[, colSums(data_y) != 0]
    
    data_x <- data_x[, colnames(data_y)] %>%
      t() %>% 
    ...
    tibble(
      ...
      pc_axis = pc_axis,
      pc_var = 0.95,
      details = 'explain_by_species'
    )
  } 
)

write.table(
  result, paste0(args[2], '.explain_by_species.tsv'),
