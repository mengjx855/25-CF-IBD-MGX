...
profile <- data.table::fread('merge_tpm.fam.tpm') |> 
...
write_tsv(
  result, 'results/1.fam.random.rep999.tsv', 
  col_names = F, quote = NULL
)

write_tsv(
  select(result, name), 'results/1.fam.random.rep999.ids', 
  col_names = F, quote = NULL
)
  
#### matched families random selection 15 ####
selected <- read_rds('results/9.feature_importance.names.rds')[1:15]
...
random_select$rep000_seed0000 <- filter(data, grepl('CFF', name)) |> 
  pull(name)
...
write_tsv(
  result, 'results/7.fam.random.rep999.15.tsv', 
  col_names = F, quote = NULL
)

write_tsv(
  select(result, name), 'results/7.fam.random.rep999.15.ids', 
  col_names = F, quote = NULL
)
